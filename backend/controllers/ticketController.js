...
const getAllTickets = async(req, res) => {
  try {
    ...
    if(req.user.role !== 'operator'){
      return res.status(403).json({message: "You do not have permissoin to view all tickets"});
    }
...
const deleteTicket = async (req, res) => {
  try {
    ...
    if(req.user.role !== 'operator'){
      return res.status(403).json({message: "you dont have permission to delete ticket"});
    }
